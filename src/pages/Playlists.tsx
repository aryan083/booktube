...
const Playlists = () => {
    ...
    const fetchPlaylists = async () => {
      try {
        ...
        if (!userData || !userData.playlists) {
          // No playlists found, return empty array
          console.log("No playlists found, setting empty array");
          setPlaylists([]);
          setLoading(false);
          return;
        }

        // Convert the JSON object structure to our array format
        const playlistsData = userData.playlists as PlaylistsData;
        console.log("Processing playlists data:", playlistsData);
  ...
  const handleSubmitPlaylist = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (fetchError && fetchError.code !== "PGRST116") {
        console.error("Error fetching current playlists:", fetchError);
        throw fetchError;
      }

      // Initialize with empty object if no playlists exist yet
      const currentPlaylists: PlaylistsData = userData?.playlists || {};
      console.log("Current playlists before update:", currentPlaylists);
      ...
      console.log("Updated playlists to save:", currentPlaylists);
      ...
      if (updateError) {
        console.error("Error updating playlists:", updateError);
        throw updateError;
      }

      console.log("Successfully saved playlists");
  ...
  const handleDeletePlaylist = async () => {
    ...
    try {
      console.log("Deleting playlist:", playlistToEdit);
      ...
      if (fetchError) {
        console.error("Error fetching playlists for deletion:", fetchError);
        throw fetchError;
      }

      if (!userData || !userData.playlists) {
        console.warn("No playlists found to delete from");
        setSubmitLoading(false);
        closeModal();
        return;
      }
      ...
      // Check if the playlist exists before trying to delete it
      if (!currentPlaylists[playlistToEdit]) {
        console.warn("Playlist to delete not found:", playlistToEdit);
      ...
      console.log("Current playlists before deletion:", currentPlaylists);

      // Delete the playlist
      delete currentPlaylists[playlistToEdit];

      console.log("Playlists after deletion:", currentPlaylists);
      ...
      if (updateError) {
        console.error("Error updating playlists after deletion:", updateError);
        throw updateError;
      }

      console.log("Successfully deleted playlist:", playlistToEdit);
