...
export default function ArticlePage() {
  ...
  // Fetch article data and initialize theme
  useEffect(() => {
  ...
  // Let's create more subtle text tint functions
  const getTextTint = () => {
    ...
    // For light mode: very minimal tint (3% saturation, 10% value)
    // For dark mode: very minimal tint (3% saturation, 97% value)
    const [h, ,] = primaryHsv;
    if (colorMode === "light") {
      return hsvToHex(h, 3, 10); // Extremely subtle dark tint
    } else {
      return hsvToHex(h, 3, 97); // Extremely subtle light tint
    }
  };

  // Extremely subtle button backgrounds with minimal opacity
  ...
  // Now update the pageStyle to use this very subtle tinted text color
  const pageStyle = {
    backgroundColor: themeColors?.background || extractedColors.primary,
    color: getTextTint(), // Very subtle tint
  };
          ...
          <div className="bg-opacity-80" style={pageStyle}>
            <div className="max-w-7xl mx-auto p-4 flex justify-between items-center">
              <div className="flex items-center gap-4">
                ...
                {/* For the BookTube title and icon - subtle tint */}
                <h2 className="modal__title flex items-center gap-2 text-lg">
                  ...
                  <span style={{ color: pageStyle.color }}>BookTube</span>
                </h2>
              </div>
