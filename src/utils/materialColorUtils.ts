...
const getColorWeight = (
  ...
  // Basic weight based on frequency in the image
  const frequencyWeight = pixelCount / totalPixels;
  
  // Convert to HSV to evaluate saturation and value
  const [, s, v] = rgbToHsv(color[0], color[1], color[2]);
  
  // Increase weight for colors with good saturation and value
  const saturationFactor = Math.min(1, s / 60); // Higher saturation is better
  const valueFactor = (v > 85 || v < 15) ? 0.5 : 1; // Avoid extreme brightness/darkness
  
  return frequencyWeight * saturationFactor * valueFactor;
};

// Detect if a color is a skin tone (to avoid using skin tones for theming)
const isSkinTone = (r: number, g: number, b: number): boolean => {
  // Convert to HSV - skin tones have specific hue and saturation ranges
