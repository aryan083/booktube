...
const UploadModal: React.FC<UploadModalProps> = ({ isOpen, onClose, onNext }) => {
                  ...
                  {!files[type] ? (
                    <>
                      <input
                        ...
                        accept=".pdf"
                        onChange={(e) =>
                          handleFileChange(type as "pdf1" | "pdf2", e.target.files?.[0] || null)
                        }
                      ...
                      <div className="flex flex-col items-center justify-center w-full">
                        <span className="inline-flex justify-center items-center size-20 bg-gray-100 text-gray-800 rounded-full dark:bg-neutral-700 dark:text-neutral-200">
                          <svg className="shrink-0 size-5" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    <div className="flex justify-between w-full">
                      <div>
                        ...
                        <span className="block text-sm text-gray-400">
                          {files[type].file.size < 1024 ? `${files[type].file.size} B` :
                           files[type].file.size < 1048576 ? `${(files[type].file.size / 1024).toFixed(2)} KB` :
                           files[type].file.size < 1073741824 ? `${(files[type].file.size / 1048576).toFixed(2)} MB` :
                           `${(files[type].file.size / 1073741824).toFixed(2)} GB`}
                        </span>
                      </div>
                      <button
                        onClick={() => {
                  ...
                  <div className="mt-2 flex items-center justify-between">
                    <div className="flex w-full h-2 bg-gray-700 rounded-full overflow-hidden">
                      <div
                        ...
                        role="progressbar"
                        style={{ width: `${files[type].progress}%`, background: 'linear-gradient(to left, #4caf50, #81c784)' }}
                        aria-valuenow={files[type].progress}
                        aria-valuemin={0}
                        aria-valuemax={100}
                      ></div>
                    </div>
                    <span className="ml-2 text-sm text-gray-400">{files[type].progress}%</span>
                  </div>
                )}
              </div>
            ))}
